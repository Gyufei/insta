  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {holders?.map((holder) => (
          <div key={holder.user_name || holder.wallet} className="flex items-center justify-between">
            {(holder.wallet || holder.user_id) ? (
              <Link 
                to={`/profile/${holder.wallet || holder.user_id}`}
                className="flex items-center gap-3 hover:text-blue-600"
              >
                {holder.user_avatar ? (
                  <img src={holder.user_avatar} alt="" className="w-8 h-8 rounded-full" />
                ) : (
