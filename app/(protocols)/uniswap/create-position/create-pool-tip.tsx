...
export function CreatePoolTip({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        'flex items-center basis-auto box-border relative min-h-0 min-w-0 shrink-0 flex-row gap-4 p-2 rounded-lg bg-[#FFF6E1] border-[#FFDD83] text-[#F3C024]',
        className
      )}
    >
      <BadgeHelpTooltip
        ...
        content="Your selections will create a new liquidity pool which may result in lower initial
          ...
          risks."
      />
      <span className="inline-block box-border whitespace-pre-wrap m-0 break-words text-xs leading-relaxed font-medium">
        Creating new pool
      </span>
    </div>
  );
