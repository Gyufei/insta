import { INadFunTokenMarketInfo } from '@/lib/data/use-nadfun-token-market-info';
...
function getMarketData(
  ...
  marketInfo: INadFunTokenMarketInfo | null | undefined
) {
  // 检查基本参数
...
export function useNadFunReceiveAmount(
  tokenAddress: string | undefined,
  marketInfo: INadFunTokenMarketInfo | null | undefined
) {
    ...
    try {
      ...
      try {
        ...
        if (amountInBig > x * BigInt(10)) {
          throw new Error(
            'Calculated input amount is unreasonably large. Please check the values.'
          );
        }
