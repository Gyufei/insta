...
export function useTwitterSign() {
  ...
  function goTwitter(cb: string) {
    window.location.href = isProduction
      ? `https://twitter.com/i/oauth2/authorize?response_type=code&client_id=NWxiS1k1WnpIVFpGdFg2YmtfQk46MTpjaQ&redirect_uri=${cb}&scope=users.read%20tweet.read%20offline.access%20space.read&state=state&code_challenge=challenge&code_challenge_method=plain`
